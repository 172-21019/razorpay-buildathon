...
const seedPath = path.resolve(__dirname, 'backend/data/seed.json');
const dbPath = path.resolve(__dirname, 'backend/data/ecommerce.db');
...
const finalSeed = [...originalSeed, ...newProducts];
...
db.serialize(() => {
  ...
  stmt.finalize();
  console.log('Finished updating DB and seed.json with', newProducts.length, 'branded products');
});
